...
const { todoModule } = await import('../index');

const app = new Elysia({ prefix: '/api' }).use(todoModule);

describe('Hexagonal Todo — API Integration', () => {
  beforeAll(async () => {
    await db
      .insert(user)
      .values({
  ...
  it('GET /api/v2/todo — should list todos', async () => {
    const res = await app.handle(new Request('http://localhost/api/v2/todo'));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('GET /api/v2/todo/:id — should get a single todo', async () => {
    const res = await app.handle(new Request(`http://localhost/api/v2/todo/${createdIds[0]}`));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('GET /api/v2/todo/:id — should return 404 with domain error for non-existent', async () => {
    const res = await app.handle(new Request('http://localhost/api/v2/todo/non-existent-id-12345'));
