...
describe('Hexagonal Todo — Use Cases (unit)', () => {
  ...
  describe('GetTodoByIdUseCase', () => {
    ...
    it('should throw TodoNotFoundError if not found', async () => {
      ...
      await expect(useCase.execute('nope', 'user-1')).rejects.toThrow(TodoNotFoundError);
    });
  });
  ...
  describe('UpdateTodoUseCase', () => {
    ...
    it("should throw TodoNotFoundError if todo doesn't exist", async () => {
      ...
      await expect(useCase.execute('nope', 'user-1', { title: 'X' })).rejects.toThrow(
        TodoNotFoundError,
      );
    });
  });
